...
export const PathFinder = () => {
  ...
  //       // console.log("and case:: parent=>", parent, "'s child=>", children);
  //       // console.log("CHECK :::: ", takenListRef.current);
  ...
  //         return newSet;
  //       });
  //       setAffectedList(prev => [...prev, parent]);
  //     }
  //   }
  // };
  ...
  const handleGood = async (classdata: string, index: number) => {
    ...
    // and then it will be used as paramter for 
    // export const TreeVisualizer2 = (data:any, userlist:string[], takenlist:string[], nextlist:string[]) => { => { ... }
    // <TreeVisualizer2 data={tree}, userlist={userSelectedTree}, takenlist ={takenListForTree}, nextlist={nextListForTree}/>
  ...
  const toggleSelection = (option: string) => {
    setSelected((prev) => {
      if (prev.includes(option)) {
        return prev.filter((item) => item !== option); // Remove if already selected
      } else {
    ...
    <div className="dashboard-container">
      ...
      <div className="second-section">
        <div className="relative inline-block">
          ...
          >
            {selected.length > 0 ? selected.join(", ") : "Select options"} {/* Show all selected */}
          </button>

          {isOpen && (
            <ul className="absolute left-0 mt-1 w-full border rounded bg-white shadow">
              {options.map((option, index) => (
                <li
                  key={index}
                  className={`px-4 py-2 cursor-pointer ${
                    selected.includes(option) ? "bg-blue-200" : "hover:bg-gray-100"
                  }`}
                  onClick={() => toggleSelection(option)}
                >
                  {option} {selected.includes(option) && "✔"} {/* Show checkmark if selected */}
                </li>
              ))}
            </ul>
          )}
